...
function bind(admin, contract) {
  contract.getState = async function (user) {
    return await getState(user, contract, options);
  }
...
async function getUsers(user, addresses) { // FIXME must break to batches of 50 addresses
  ...
  const results = await searchUntil(user, contract, predicate, { config, query: { address: `in.${csv}` } });
  return results;
}
...
async function getUser(user, username) {
  ...
  const response = (await searchUntil(user, contract, predicate, { config, query: { username: `eq.${username}` } }))[0];
  return response;
}

async function getUserByAddress(user, address) {
  function predicate(response) {
    if (response.length >= 1) {
      return response;
    }
  ...
  const response = (await searchUntil(user, contract, predicate, { config, query: { address: `eq.${address}` } }))[0];
  return response;
}

...
import * as permissionedHashmapJs from '../permissionedHashmapSA';
...
describe('PermissionedHashmapSA tests', function () {
  ...
  it('get', async function () {
    ...
    await contract.put(args);
    // get
    const value = await contract.get({ stringKey: args.stringKey, addressKey: args.addressKey });
    assert.equal(parseInt(value), parseInt(args.value), 'value');
    const notFound = await contract.get({ stringKey: '666', addressKey: 'deadbeef' });
  ...
  it('contains', async function () {
    ...
    await contract.put(args);
    // contains
    const result = await contract.contains({ stringKey: args.stringKey, addressKey: args.addressKey });
    assert.equal(result, true, 'contains: true');
    const notFound = await contract.contains({ stringKey: '666', addressKey: 'deadbeef' });
  ...
  it('remove', async function () {
    ...
    const args = { stringKey: putArgs.stringKey, addressKey: putArgs.addressKey }
    // contains
    {

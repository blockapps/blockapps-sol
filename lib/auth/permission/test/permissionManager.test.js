...
describe('PermissionManager tests', function () {
  ...
  it('Grant (address with permissions)', async function () {
    ...
    const uid = util.uid()
    const args = await createPermitArgs(adminCredentials, uid)
  ...
  it('Get permit', async function () {
    ...
    const uid = util.uid()
    const permitArgs = await createPermitArgs(adminCredentials, uid)
  ...
  it('Transfer Ownership - AUTHORIZED', async function () {
    const newOwner = await createUser(admin, options)
    const contract = await permissionManagerJs.uploadContract(admin, master)
  ...
  it('Transfer Ownership - positive case', async function () {
    const uid = util.uid()
    const newOwner = await createUser(master, options)
    const contract = await permissionManagerJs.uploadContract(admin, master)
    // admin works
    const args = await createPermitArgs(admin, uid)
  ...
  it('Transfer Ownership - UNAUTHORIZED', async function () {
    ...
    {
      const attacker = await createUser(admin, options)
  ...
  it('EventLog - Check permissions', async function () {
    ...
    const uid = util.uid()
    const permitArgs = await createPermitArgs(admin, uid)
    await contract.grant(permitArgs)
...
async function createPermitArgs(credentials, uid) {
  const user = await createUser(credentials, { config })
